...
- **Application icon** (`resources/icons/app-icon-*.png`,
  `resources/icons/app-icon.ico`) — a composed badge (blue rounded
  square + the Tabler "wind" glyph, tagged "breeze" in its own metadata —
  a direct thematic match for "Zephyr"), not a bare Tabler icon, since a
  ...
  of 256) before this was corrected; the fixed version has 33. The
